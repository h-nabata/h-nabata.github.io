(function(global){
  ...
  function settings(){const path=$('generationKind').value==='path';$('pathControls').hidden=!path;$('randomControls').hidden=path;$('randomRingOptions').hidden=$('randomMode').value==='orientation';$('generationCount').min=path?2:1;}
  ...
  const formKey=()=>JSON.stringify(['generationKind','generationCount','generationSeed','randomMode','pathMethod','pathStart','pathEnd','randomRingAngle'].map(id=>$(id).value).concat($('pathMinimumImage').checked,$('randomRings').checked));
  function run(){
    if(job)return;
    try{
      if(/未適用|編集中/.test($('xyzEditStatus').textContent))throw Error('座標編集欄の未適用データを先に適用するか、「現在の構造を再表示」を押してください。');
      const kind=$('generationKind').value,options={count:Number($('generationCount').value),seed:Number($('generationSeed').value),mode:$('randomMode').value,method:$('pathMethod').value,minimumImage:$('pathMinimumImage').checked,rings:$('randomRings').checked,ringAngle:Number($('randomRingAngle').value)};
      ...
      try{
        j.worker=new Worker('./generation-worker.js?v=15');j.timeout=setTimeout(()=>{if(job===j){finish(j);message('生成が120秒を超えました。原子数・構造数を減らしてください。');}},120000);
        j.worker.onmessage=e=>{
          ...
          const r=e.data.result;finish(j);const next=MV.Model.createState(r.state);if(kind==='random'&&app().getState().metadata.measurements){next.metadata.measurements=JSON.parse(JSON.stringify(app().getState().metadata.measurements));next.metadata.measurements.items.forEach(m=>{m.bindings={};});}next.viewSettings={...app().getState().viewSettings};app().change('generated structures',()=>next,true);$('dataReset').click();$('generationDialog').close();app().setStatus(r.message+' 「戻す」で生成前へ戻れます。');$('viewer').scrollIntoView({block:'center'});
        };
  ...
  function init(){
    $('btnRandomStructures').addEventListener('click',()=>open('random'));$('btnReactionPath').addEventListener('click',()=>open('path'));$('generationKind').addEventListener('change',settings);$('randomMode').addEventListener('change',settings);$('generationRun').addEventListener('click',run);$('generationCancel').addEventListener('click',cancel);$('generationDialog').addEventListener('close',cancel);
